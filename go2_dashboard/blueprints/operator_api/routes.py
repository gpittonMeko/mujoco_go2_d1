...
def api_hermes_command() -> Any:
    ...
    server_notes = [str(x) for x in list(lex_notes) + list(inject_notes) if str(x).strip()]
    if server_notes:
...
def api_arm_servo_snapshot() -> Any:
    ...
    if cur is None:
        out: dict[str, Any] = {
            ...
            "hint": "Verifica bin/d1_arm_feedback_helper e DDS sul robot.",
        }
        if want_diag:
...
def api_arm_true_zero() -> Any:
    ...
    if op == "goto_saved_start":
        result = goto_saved_start_from_json(delay_ms=_arm_post_delay_ms(body))
        payload, code = _true_zero_motion_http_response_lite(result)
        return jsonify(payload), code
...
def api_arm_file_poses_status() -> Any:
    """Stato dei file ``true_zero_pose.json`` e ``start_alignment.json`` sulla macchina."""
    tz = TRUE_ZERO_POSE_PATH.is_file()
    st = ALIGNMENT_START_PATH.is_file()
    return jsonify(
        {
            "ok": True,
            "true_zero_pose_json": tz,
            "true_zero_pose_path": str(TRUE_ZERO_POSE_PATH),
            "start_alignment_json": st,
...
def api_arm_goto_saved_start() -> Any:
    ...
    out = goto_saved_start_from_json(delay_ms=_arm_post_delay_ms(body))
    code = 200 if out.get("ok") else 503
    return jsonify(out), code
...
def api_arm_goto_zero_then_start() -> Any:
    ...
    out = goto_true_zero_then_saved_start_from_json(delay_ms=_arm_post_delay_ms(body))
    code = 200 if out.get("ok") else 503
    return jsonify(out), code
