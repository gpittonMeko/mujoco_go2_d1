...
def reconcile_d1_sdk_daemons() -> dict[str, Any]:
    """Chiude daemon tracciato + eventuali ``d1_sdk_command`` orfani (due publisher DDS → braccio che cede)."""
    stop_command_daemon()
    killed: dict[str, Any] = {"ok": True, "skipped": True, "reason": "pkill_unavailable"}
    if os.name != "nt":
        try:
            r = subprocess.run(
                ["pkill", "-f", "d1_sdk_command"],
                capture_output=True,
                text=True,
                timeout=2.0,
            )
            killed = {"ok": r.returncode in (0, 1), "returncode": r.returncode}
        except Exception as exc:
            killed = {"ok": False, "detail": repr(exc)}
    # Il publisher DDS è stato distrutto: la coppia va RI-asserita (funcode 5) sul nuovo daemon,
    # altrimenti i motori restano molli e il braccio cede. Resettiamo lo stato couple.
    ...
    alive = ensure_command_daemon(motion_profile.daemon_delay_ms())
    recouple = arm_couple_once(force=True) if alive else {"ok": False, "skipped": True}
    return {
        "ok": alive,
        "action": "reconcile_d1_sdk_daemons",
        "pkill": killed,
        "recouple": recouple,
    }


def stream_torque_during_sleep(servo_deg: list[float], duration_s: float) -> dict[str, Any]:
    """Durante attese movimento: funcode 2 periodici (come slider live con ``keep_lock``)."""
    sd = clamp_servo_deg(servo_deg)
    if duration_s <= 0.02 or not _real_arm_enabled():
        return {"ok": True, "skipped": True, "reason": "no_sleep", "action": "stream_torque_sleep"}
    if not _arm_coupled:
        return {"ok": True, "skipped": True, "reason": "not_coupled", "action": "stream_torque_sleep"}
    try:
        tick_s = float(os.environ.get("D1_GRASP_HOLD_TICK_S", "0.07"))
    except ValueError:
        tick_s = 0.07
    tick_s = max(0.04, min(0.2, tick_s))
    deadline = time.monotonic() + float(duration_s)
    ticks = 0
    last: dict[str, Any] = {"ok": True, "skipped": True}
    while time.monotonic() < deadline:
        last = jog_pose_deg(sd, keep_lock=True)
        ticks += 1
        if not (last.get("ok") or last.get("skipped")):
            break
        time.sleep(tick_s)
    return {
        "ok": bool(last.get("ok") or last.get("skipped")),
        "action": "stream_torque_sleep",
        "ticks": ticks,
...
def publish_trajectory_stream(
    messages: list[dict[str, Any]], *, delay_ms: int | None = None
) -> dict[str, Any]:
    """Traiettoria/hold sul daemon DDS persistente (come il jog).

    A differenza di ``_publish_messages`` (processo one-shot che crea e distrugge un
    writer DDS, facendo perdere la *liveliness* al firmware D1 → cedimento motori), qui i
    waypoint passano sul publisher persistente sempre vivo, con cadenza ``delay_ms`` lato
    Python. Il daemon NON viene mai chiuso: la coppia resta tenuta tra una fase e l'altra.
    """
    if not messages:
        return {"ok": True, "count": 0}
    if not _real_arm_enabled():
    ...
    if not ensure_command_daemon(None):
        return {"ok": False, "reason": "daemon_start_failed"}
    pace_s = max(0.0, float(delay_ms) / 1000.0) if delay_ms else 0.0
    sent = 0
    for idx, msg in enumerate(messages):
        pub = publish_messages_stream([msg])
        if not (pub.get("ok") or pub.get("skipped")):
            return {**pub, "sent": sent, "backend": "d1_sdk_stream"}
        sent += 1
        if pace_s and idx < len(messages) - 1:
            time.sleep(pace_s)
    return {"ok": True, "count": sent, "stream": True, "backend": "d1_sdk_stream"}
...
def arm_motion_session_begin(*, with_power: bool | None = None) -> dict[str, Any]:
    """Avvio sessione come d1_jog (5053): daemon DDS persistente + coppia + hold sulla posa attuale.

    Non usa processi one-shot ``_publish_messages`` (perdita liveliness → braccio che cede).
    """
    global _arm_coupled, _couple_last_ts
    from go2_dashboard.d1_jog import motion_profile
    ...
    if with_power is None:
        with_power = _truthy_env("GO2_GRASP_ARM_WITH_POWER", "0")
    dm = motion_profile.daemon_delay_ms()
    if not ensure_command_daemon(dm):
        return {"ok": False, "reason": "daemon_start_failed", "action": "arm_motion_session_begin"}
    daemon_ok = _cmd_daemon_proc is not None and _cmd_daemon_proc.poll() is None
    if _arm_coupled and daemon_ok:
        fb = read_servo_deg(fast=True)
        if fb.get("ok") and fb.get("servo_deg"):
            cur = clamp_servo_deg(fb["servo_deg"])
            set_servo_cache(cur)
            hold = hold_pose_stream(servo_deg=cur)
            return {
                "ok": True,
                "action": "arm_motion_session_begin",
                "daemon_alive": True,
                "arm_coupled": True,
                "skipped_boot": True,
                "snapshot_deg": cur,
                "hold": hold,
                "backend": "d1_sdk_stream",
            }
    seq = int(time.time()) % 100000
    boot: list[dict[str, Any]] = []
    if with_power:
        boot.append({"seq": seq, "address": 1, "funcode": 6, "data": {"power": 1}})
        seq += 1
    if not _arm_coupled:
        boot.append(_couple_enable_message(seq))
    if boot:
        pub = publish_messages_stream(boot, delay_ms=motion_profile.stream_delay_ms())
        if not (pub.get("ok") or pub.get("skipped")):
            return {**pub, "action": "arm_motion_session_begin"}
        _arm_coupled = True
        _couple_last_ts = time.time()
    fb = read_servo_deg(fast=False)
    if not fb.get("ok") or not fb.get("servo_deg"):
        return {
            "ok": False,
            "reason": fb.get("reason", "no_servo_feedback"),
            "action": "arm_motion_session_begin",
            "daemon_alive": True,
        }
    cur = clamp_servo_deg(fb["servo_deg"])
    ...
    return {
        ...
        "action": "arm_motion_session_begin",
        "daemon_alive": True,
        "arm_coupled": bool(_arm_coupled),
        "with_power": bool(with_power),
        "snapshot_deg": cur,
        "hold": hold,
        "backend": "d1_sdk_stream",
    }
...
def move_servo_deg_jog_trajectory(
    ...
    *,
    ...
    """Goto/traiettoria giunti — stesso percorso di ``program_runner.move_to_servo_deg_smooth``."""
    from go2_dashboard.d1_jog import motion_profile, program_runner
    ...
    if not _arm_coupled:
        ...
        if d1_arm_motion.is_live_session_active():
            return {
                "ok": False,
                "reason": "not_coupled",
                "hint": "Coppia ON (tab Giunti) prima del goto",
                "action": "move_servo_deg_jog_trajectory",
            }
        wp = _truthy_env("GO2_GRASP_ARM_WITH_POWER", "0")
        prep = arm_motion_session_begin(with_power=wp)
        if not prep.get("ok"):
            return {**prep, "action": "move_servo_deg_jog_trajectory"}
    fb = read_servo_deg(fast=True)
    if not fb.get("ok") or not fb.get("servo_deg"):
        return {"ok": False, "reason": fb.get("reason", "no_feedback"), "action": "move_servo_deg_jog_trajectory"}
    cur = fb["servo_deg"]
    target = clamp_servo_deg(target_servo_deg)
    step = max_step_deg if max_step_deg is not None else max(1.5, float(os.environ.get("D1_GRASP_JOINT_STEP_DEG", "2.5")))
    waypoints = program_runner.plan_joint_waypoints(cur, target, max_step_deg=step)
    delay_ms = motion_profile.stream_delay_ms()
    if not ensure_command_daemon(delay_ms):
        return {"ok": False, "reason": "daemon_start_failed", "action": "move_servo_deg_jog_trajectory"}
    sent = 0
    for i, sd in enumerate(waypoints):
        msg = _pose_message(sd, seq=int(time.time()) % 100000 + i)
    ...
    settle_s = program_runner._estimate_move_duration_s(cur, target)
    stream_torque_during_sleep(target, settle_s)
    hold = hold_pose_stream(servo_deg=target)
    wait = program_runner.wait_until_at_target(target)
    out: dict[str, Any] = {
        "ok": bool(wait.get("ok")),
        "action": "move_servo_deg_jog_trajectory",
        "path_points": len(waypoints),
        "target_servo_deg_7": target,
        "stream": True,
        "backend": "d1_sdk_stream",
        "traj_delay_ms": delay_ms,
        "wait_at_target": wait,
        "hold": hold,
    }
    if not wait.get("ok"):
        out["reason"] = wait.get("reason", "wait_failed")
    return out
