...
class D1RgbAndMotorUiTests(unittest.TestCase):
    ...
    def test_ui_has_rgb_badges_and_full_motor_panel(self) -> None:
        ...
        self.assertEqual(src.count('data-teach-section='), 3)
        ...
        self.assertIn('Memorizza presa + HOLD', src)
        ...
        self.assertIn('Inizia nuova calibrazione 6D', src)
        self.assertNotIn('/api/pick/metric/calibration/target.pdf', src)
        self.assertNotIn('Target ArUco singolo di riserva', src)
        ...
        self.assertIn('AUTO CALIBRAZIONE 6D SOSPESA', src)
        self.assertIn('Auto movimento disabilitato', src)
        ...
        self.assertIn('FLUSSO REALE: segui solo questi passi', src)
        self.assertIn('1. Inizia nuova calibrazione 6D', src)
        self.assertIn('2. Avvia guida live', src)
        self.assertIn('3. Release: lo sostengo io', src)
        self.assertIn('4. Salva sample + HOLD', src)
        self.assertIn('5. Calcola hand-eye 6D', src)
        self.assertIn('Controllo opzionale', src)
        self.assertIn('Verifica AprilGrid senza salvare', src)
    ...
    def test_arm_runtime_and_daemon_regressions_are_guarded(self) -> None:
        ...
        self.assertIn('"base_source": "scan_left"', app)
