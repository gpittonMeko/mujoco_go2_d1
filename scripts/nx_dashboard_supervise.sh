#!/bin/bash
# Supervisore: rilancia serve_dashboard_modular.py se termina (crash, OOM, eccezioni).
...
while true; do
  echo "$(date -Is) exec serve_dashboard_modular.py"
  python3 scripts/serve_dashboard_modular.py >> dashboard_run.log 2>&1
  ex=$?
  echo "$(date -Is) serve_dashboard_modular.py exited code=${ex} — sleep ${RESTART_SEC}s"
  sleep "$RESTART_SEC"
done
